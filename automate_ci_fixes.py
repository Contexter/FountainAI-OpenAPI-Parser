...
] + [os.path.join(TEST_DIR, f) for f in os.listdir(TEST_DIR) if f.endswith(".py")]

# Patterns to match
DICT_PATTERN = re.compile(r'(\w+)\.dict\((.*?)\)')
MOCK_PATTERN = re.compile(r'import unittest(\n|[^.]*)')  # Match for unittest.mock import
    ...
    with fileinput.FileInput(file_path, inplace=True, backup='.bak') as file:
        for line in file:
            if IMPORT_PATTERN.search(line):
                print(line.replace('import unittest', 'from unittest.mock import patch, Mock'), end='')
            else:
                print(line, end='')
    ...
    with fileinput.FileInput(file_path, inplace=True, backup='.bak') as file:
        ...
        for line in file:
            ...
            if in_class and "paths:" in line and "Dict[str, PathItem]" in line:
                line = line.replace("Dict[str, PathItem]", "Dict[str, PathItem] = Field(default_factory=dict)")
                in_class = False
            print(line, end='')
    ...
    with fileinput.FileInput(file_path, inplace=True, backup='.bak') as file:
        for line in file:
            if "open(file_path, 'r')" in line:
                line = line.replace("open(file_path, 'r')", "open(file_path, 'r') if os.path.exists(file_path) else None")
            print(line, end='')
