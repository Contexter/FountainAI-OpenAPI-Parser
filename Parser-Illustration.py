...
parsing_edges = [
    ...
    (
        "Serialization Module",
        "Input OpenAPI Spec",
    ),  # Complete the circle to represent iterative process
]

# Define edges representing connections with the APIs
api_edges = [
    ("Ensemble Service", "Core Script Management API"),
    ("Ensemble Service", "Character Service"),
    ("Ensemble Service", "Central Sequence Service"),
    ("Ensemble Service", "Action Service"),
    ("Ensemble Service", "Story Factory API"),
    ("Ensemble Service", "Spoken Word Service"),
    ("Ensemble Service", "Session and Context Management API"),
    ("Ensemble Service", "Performer Service"),
    ("Ensemble Service", "Paraphrase Service"),
...
node_colors = [
    (
        "lightyellow"
        if node == "Input OpenAPI Spec"
        else (
            "lightgreen"
            if node
            in ["YAML/JSON Loader", "Reference Resolver", "Validation Module"]
            else (
                "skyblue"
                if node
                in [
                    ...
                    "lightcoral"
                    if node == "Ensemble Service"
                    else (
                        "lightgreen"
                        if node == "FountainAI OpenAPI Parser"
                        else "lightblue"
                    )
                )
            )
