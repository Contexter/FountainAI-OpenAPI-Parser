...
def fix_test_parser_file():
    ...
    content = re.sub(r"self\.assertIn\(\"/example\", parsed\.paths\)",
                     "    self.assertIn(\"/example\", parsed.paths)", content)
...
def run_tests():
    print("Running tests...")
    subprocess.run(["PYTHONPATH=$(pwd)", "python", "-m", "unittest", "discover", "-s", "tests"], shell=True)
