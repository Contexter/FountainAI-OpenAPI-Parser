...
def update_config(filename):
    ...
    new_content = content.replace("allow_population_by_field_name", "populate_by_name")
...
def update_schema_field(filename, class_names, old_field="schema", new_field="schema_data"):
    with open(filename, 'r') as file:
    ...
    for line in lines:
        ...
        if within_class and re.search(rf'\b{old_field}\s*:', line):
            ...
            print(f"Updated '{old_field}' to '{new_field}' in class '{within_class}' in {filename}")
        else:
